...
By introgressing the highland inverted haplotype from the Mexican traditional variety Mi21 into B73 through eight generations of backcrossing, generating Near Isogenic Lines (NILs), we reduced the confounding Mi21 linkage drag to 24 Mb around the inversion.
These NILs were phenotyped across three field environment-year combinations, two years in Pennsylvania and one in North Carolina.
...
\invfour has also been associated to phosphorus defficiency adaptation. To test this effect, we field evaluated \invfour NILs under different levels of phosphorus and the results are presented in a companion study~\cite{rodriguez2026phosphorus}.

In this study, we used new, high quality genome assemblies of highland materials including Mi21 NILs to precisely determine \invfour breakpoints. \invfour breakpoints overlap chromosomal knob repeat arrays, consistent with an ectopic recombination origin. 
We then show that \invfour modulates flowering and plant height across environments, with genotype-by-environment interactions congruent with local adaptation. Microscopy, morphological analysis confirmed that \invfour NILs have larger shoot apical meristems than non inverted haploypes, a developmental pattern associated with faster flowering. (Citations). We then used transcriptomic analysis to identify a JUMONJI histone demethylase cluster with copy number variation between inverted  and non-inverted \invfour haplotyes that is reflected in significant differences of expression that could affect other genes within and outside \invfour by epigenetic regulation. 
...
This evidence suggests that \invfour regulates growth by rewiring gene networks invoved in growth regulation probably through chromatin modification. These molecular mechanisms support the adaptive value of \invfour in highland conditions, where it promotes faster growth and earlier flowering 
...
The localization of \invfour breakpoints to gene-poor tandem repeat regions parallels findings in other systems where inversion breakpoints preferentially occur at repetitive, low-gene-density regions such as centromeric satellite arrays and segmental duplications \cite{Harringmeyer2022-kv, araya2025, Gozashti2025-ya}
...
\subsection{Divergent highland introgression is mostly confined to a 39 Mb segment spanning \invfour}
...
\caption{\textbf{WGCNA module perturbation analysis shows network disruption in \invfour plants.}
...
To characterize the breakpoint regions, we compared the genome sequences of TIL18, a \mex inbred line, PT, a Mexican highland traditional variety, a Mi21 NIL selected for \invfour, and B73, the reference genome homozygous for the standard karyotype (Table~\ref{tab:breakpoints}).
...
More broadly, repetitive sequences at inversion breakpoints appear to be a general feature across taxa. In deer mice (\textit{Peromyscus maniculatus}), 21 megabase-scale inversion polymorphisms are flanked by long inverted repeats (0.5–50 kb) and enriched for segmental duplications at their breakpoints, consistent with an ectopic recombination origin \cite{Harringmeyer2022-kv}. 
Chromosome-level assemblies of four deer mouse subspecies further revealed that breakpoints frequently map to centromeric satellite arrays, which may serve as repeat-rich "safe havens" where rearrangements avoid disrupting gene-dense regions \cite{Gozashti2025-ya} — a pattern reminiscent of the gene-poor knob repeat arrays we find at the \invfour breakpoints
...
Introgressed segments are expected to produce local DE, most likely downregulation, as demonstrated in \textit{Solanum} introgression lines \cite{guerrero2016}, but  upregulation has recently been reported in maize and linkked with reduced sRNA silencing \cite{minow2021}.
...
In rice, knockdown of OsJMJ703 accelerates flowering \cite{song2019-jm} Song et al. (2018), mirroring the direction of the \invfour effect, where reduced JMJ dosage co-occurs with earlier anthesis. 
...
This model is supported by recent work showing that OsJMJ703 physically associates with the Mediator complex subunit OsMED23 and the transcription factor OsWOX3A to form a transcriptional repressor complex that modulates H3K4me3 at growth-related target genes \cite{Huang2025-pd}. 
This finding demonstrates that JMJ703 orthologs can influence gene expression through multi-protein complexes, consistent with the hub-like coexpression behavior we observe for jmj2 and jmj4 in CTRL networks.
...
Whether JMJ drives the network disruption or both reflect independent effects of the same structural variant will require ChIP-seq for H3K4me3 and fine-mapping to separate inversion effects from flanking linkage drag, and ultimately reversion of the inverted haplotype to allow recombination inside \invfour.
...
To assess the effect of \invfour across environments, we analyzed phenotype data from three location-year combinations: Pennsylvania 2022 (PA2022), Pennsylvania 2025 (PA2025), and North Carolina 2025 (NC2025).
