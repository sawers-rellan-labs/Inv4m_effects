...
%%% Abstract %%%%%%%%%%%%%%%%%%
\begin{abstract}
Maize adaptation to temperate zones may have been facilitated by introgression from highland wild relatives.
We investigated the effects of \invfour, a 15 Mb chromosomal inversion from \textit{Zea mays ssp. mexicana}, by introgressing it into temperate maize (B73) through eight generations of backcrossing.
Field trials under contrasting phosphorus treatments revealed that \invfour significantly accelerates flowering time and increases plant height, independent of phosphorus availability.
Transcriptomic analysis identified perturbations in a cell proliferation gene expression network, including downregulation of JUMONJI histone demethylases that show higher copy numbers in modern maize compared to highland teosinte.
Phenotypic measurements confirmed that \invfour plants exhibit elongated shoot apical meristems.
While classical phosphorus starvation response genes were detected within the inversion, \invfour showed no detectable effect on phosphorus starvation responses.
These findings illuminate the genetic mechanisms underlying \invfour's contribution to local adaptation and identify candidate genes driving its phenotypic effects.
\end{abstract}
%%%%%%%%%%%%%%%%%%%%%%%%%%
...
For example, the phosphate transporter gene \textit{ZmPho1;2a} (\textit{phos2}, \textit{Zm00001eb191650}), located within \invfour, is a strong candidate for adaptation to low phosphorus availability \cite{salazar-vidal2016-rl}.
...
To achieve this, we backcrossed MICH21, a Mexican highland TV carrying \invfour, into the B73 genetic background for eight generations, generating Near Isogenic Lines (NILs) \cite{crow2020}.
...
We observed that \invfour significantly reduces flowering time and increases plant height, independent of phosphorus levels.
We identified a cluster of JUMONJI histone demethylases, which have higher copy numbers in modern maize compared to highland teosinte and TVs, as potential contributors to flowering time differences.
Additionally, we found a group of cell cycle-related genes that may underlie height differences.
...
We detected typical tandem array patterns in the dotplots (blue and red rectangles in Fig~\ref{fig::design}B) and we took a closer look at the available repeat annotation of B73.
...
These chromosomal knob 180 bp and TR-1 repeat annotations matched the observed tandem array patterns observed in the dotplots Fig~\ref{fig::design}B.
...
We found in the long arm of chromosome 4 repeat arrays corresponding to previously reported heterochromatic knobs, the ones interspersed in the \invfour breakpoints, and an internal knob repeat array inside \invfour Fig~\ref{fig::design}A.
...
TIL18 has a 30 Mb region of mixed repeats likely corresponding to the 4L knob reported in \mex \cite{bilinski2018,albert2010}, PT is knobless, and B73 carries a 4L knob consisting of \textit{TR-1} repeats \cite{ghaffari2013,albert2010} (Fig~\ref{fig::design}A).
...
In each genome, the breakpoint segments on each side of the inversion showed asymmetries in size, knob repeat number, presence of knob inverted repeat arrays, and closeness of the knob repeats to the \invfour anchor gene (Fig~\ref{fig::design}B).

In particular, in B73 the upstream breakpoint segment is larger (323 kb vs 89 kb), has a greater number of repeats (352 vs 26), presence of inverted knob repeat arrays (blue and red rectangles in Fig~\ref{fig::design} B Upstream), and some knob repeats close to the inversion, while the downstream breakpoint segment is smaller, has a lower number of repeats, no inverted repeats of knob sequences (just red rectangles in Fig~\ref{fig::design}B, Downstream), and the few knob repeats present are closer to the downstream external anchor gene rather than to \invfour.
...
We converted this genotype matrix to identical genotype run \cite{layer2016} length in base pairs to estimate the proportion of the plant genome introgressed from Mi21 (Fig~\ref{fig::design}F and G).
...
With our selection for the highland allele of PZE04175660223 we have produced plants with the introgression of the target \invfour inversion surrounded by 24 Mb of lingering unintended linkage drag (Fig~\ref{fig::design}F and G).
...
Field trials demonstrated that \invfour significantly affects both flowering time and plant architecture (Fig~\ref{fig:phenotypes}A).
Spatially-corrected phenotype analysis revealed that plants carrying the highland inversion allele flowered $1.30 \pm 0.27$ days earlier to anthesis ($p = 7.9 \times 10^{-6}$) and $1.05 \pm 0.22$ days earlier to silking ($p = 1.4 \times 10^{-5}$) compared to control lines.
...
Importantly, the shape metrics capturing SAM elongation were significantly altered: the height-to-radius ratio increased by $0.158 \pm 0.074$ ($p = 0.038$) and the shape factor ($h/r^2$) increased by $2.49 \pm 1.14$ nm$^{-1}$ ($p = 0.035$), representing a 9\% elongation of the meristem dome.
These findings suggest that the accelerated flowering and increased plant height in \invfour lines may originate from altered meristem geometry established early in development.
...
To characterize the architectural basis of \invfour effects on plant height, we measured internode lengths in the CLY2025 field experiment.
...
Total node count did not differ significantly between genotypes within the MI21 background, suggesting that \invfour effects on plant height are mediated by internode elongation rather than increased phytomer number.
...
Notably, there was no significant difference in DEG density between \invfour proper and the flanking regions (odds ratio = 1.02, $p = 0.95$), indicating that the transcriptional perturbation extends throughout the introgressed segment.
...
We classified edges as ``Reference'' if present in the MaizeNetome database (16 edges, 2.9\%) or ``Novel'' if specific to our \invfour network (536 edges, 97.1\%).
...
The Novel network revealed a previously uncharacterized \jmjiv neighborhood subgraph (Fig~\ref{fig::transnetwork}A, right panel).
...
This connection suggests that the JMJ cluster may influence cell division through post-translational mechanisms beyond chromatin modification.
...
However, microsynteny analysis across five \textit{Zea} genomes revealed that the B73 reference contains a tandem expansion of five JMJ paralogs (\jmjix, a putative pseudogene, \jmjvi, \jmjii, and \jmjiv), whereas highland teosinte TIL18, the landrace Palomero Toluque\~{n}o, and the tropical inbreds CML457 and CML459 each retain only a single \textit{jmj6} ortholog (Fig~\ref{fig:jmj_cluster}B).
The high sequence identity among paralogs ($>$97\% cDNA similarity; Fig~\ref{fig:jmj_cluster}C) suggests recent duplication and complicates individual paralog quantification.
...
Introgression of \invfour from Mexican highland maize into B73 produces consistent effects on flowering time and plant height, independent of phosphorus availability (Fig~\ref{fig:phenotypes}).
Transcriptomic analysis identifies the JUMONJI histone demethylase cluster as the strongest candidate, showing both copy number variation between highland and lowland genomes (Fig~\ref{fig:jmj_cluster}B) and expression levels below copy number expectation in field conditions (Fig~\ref{fig:jmj_cluster}A).
WGCNA reveals that \invfour introgression disrupts 12 of 13 coexpression modules (Table~\ref{tab::preservation}), with JMJ genes losing hub status (Fig~\ref{fig:wgcna}C).
The phosphorus starvation hypothesis was tested and rejected in our companion study~\cite{rodriguez2026phosphorus}; here we focus on identifying candidate genes underlying the observed adaptive phenotypes.
...
The breakpoint segments are enriched for chromosomal knob repeats: the upstream breakpoint contains 438 \textit{knob180} and 29 \textit{TR-1} repeats (74\% of annotated repeats), while the downstream breakpoint contains 37 \textit{knob180} repeats (46\%; Fig~\ref{fig::design}B).
...
This pattern is characteristic of local adaptation, where alleles beneficial in one environment may be neutral or deleterious in others \cite{kirkpatrick2006b}.
...
The \jmjii/\jmjiv cluster exhibited the largest expression difference among all DEGs, with $\log_2 \text{FC} = -3.49$ ($\textit{FDR} < 10^{-20}$; Table~\ref{tab:FT_PH_candidates}).
This downregulation was consistent across all 13 tissue-experiment combinations examined, including field and growth chamber samples spanning roots, leaves, and shoot apical meristems (Fig~\ref{fig:jmj_cluster}A).

Microsynteny analysis revealed that B73 contains five JMJ paralogs while highland teosinte TIL18 and the landraces PT, CML457, and CML459 each retain only a single ortholog (Fig~\ref{fig:jmj_cluster}B).
The high sequence identity among B73 paralogs ($>$97\%; Fig~\ref{fig:jmj_cluster}C) indicates recent tandem duplication.
Under simple dosage scaling, \invfour expression should equal CTRL/5; however, field samples showed only 49 to 61\% of this expectation (linear contrast $t$-test, $\textit{FDR} < 0.05$ for all four tissues).
Growth chamber data were less consistent (48 to 83\% of expectation, 4/9 tissues significant), suggesting environment-specific regulatory mechanisms beyond baseline copy number effects.
...
SAM measurements from two-week-old seedlings revealed that \invfour plants have elongated meristems, with a 9\% increase in the shape factor ($h/r^2 = +2.49 \pm 1.14$ nm$^{-1}$, $p = 0.035$; Fig~\ref{fig:phenotypes}C).
This early developmental change may underlie the subsequent height and flowering differences observed in field-grown plants.
...
The trans-regulatory network connects JMJ genes to cell proliferation factors including \textit{pcna2} and \textit{sec6} (Fig~\ref{fig::transnetwork}A), suggesting a mechanistic chain: JMJ downregulation $\rightarrow$ altered H3K4 methylation $\rightarrow$ growth gene misexpression $\rightarrow$ SAM elongation $\rightarrow$ accelerated flowering and increased height.
...
Our results remain correlative: 24 Mb of flanking introgression persists after eight backcross generations (Fig~\ref{fig::design}F, G), only B73 was tested as the recurrent parent, and the high sequence identity among JMJ paralogs ($>$97\%; Fig~\ref{fig:jmj_cluster}C) complicates individual gene attribution.
Functional validation through heterologous overexpression of JMJ genes in Arabidopsis, ChIP-seq for H3K4me3, and Hi-C analysis of 3D chromatin organization on chromosome 4 would strengthen the mechanistic model proposed here.
...
Edges present in MaizeNetome were classified as ``Reference'' (representing conserved co-expression relationships detected across diverse maize tissues and genotypes), while edges absent from MaizeNetome were classified as ``Novel'' (\invfour-specific).
Of the 552 edges in the \invfour trans-regulatory network, 16 (2.9\%) were Reference edges and 536 (97.1\%) were Novel edges, indicating that \invfour creates largely novel co-expression relationships not observed in the broader maize transcriptome.
