...
\author[$1$,*]{Fausto Rodríguez-Zapata}
...
A previous analysis of teosinte populations using the Maize 50K chip inferred that \invfour spans 13 Mb, is predominantly found in teosinte \mex populations \citep{pyhajarvi2013} and is introgressed into sympatric highland maize populations \citep{pyhajarvi2013,calfee2021-mr,hufford2013-gs}.
...
Previous research has shown that \invfour-highland upregulates photosynthesis genes in response to cold at the seedling stage \citep{crow2020} and is associated with earlier flowering in the Mexican highlands, which likely enhances fitness in environments with limited growth-degree accumulation throughout the year \citep{romero_navarro2017-cn, eagles1994, giauffret2000}.
...
We phenotyped these NILs across three field environment-year combinations in Pennsylvania and North Carolina, measured shoot apical meristem morphology inn seedlings, and used RNA-seq from field experiments  to dissect the regulatory architecture of the \invfour effect using weighted gene co-expression network analysis and \textit{trans}-regulatory network reconstruction. 
...
De novo genome assembly of (Mi21) \invfour and other highland genomes containing \invfour showed that \invfour carries a single ancestral copy of JMJ while temperate inbreds like B73 carry a tandem repeat expansion of five JMJ paralogs.
...
Each point represents a BLAST+ \citep{camacho2009} hit (normalized match score).
...
\textbf{(C)} AnchorWave \citep{song2022} whole-genome alignment dotplots of Mi21 NIL chromosome~4 versus TIL18 (teosinte \mex), PT (highland maize), and B73 (reference).
...
\textbf{(D)} Sequence self-similarity dotplots, using LAST \citep{kielbasa2011}, for the inversion breakpoint segments as defined in Table~\ref{tab:breakpoints}.
...
We assembled the Mi21 NIL (Fig~\ref{fig::design}A) genome from PacBio HiFi reads ($\sim$14x coverage) using hifiasm \citep{cheng2021} and scaffolded contigs against B73 and PT references with RagTag \citep{alonge2022} (N50 = 13.7 Mb; 2.2 Gb total).
Chromosome~4 was represented by a 229 Mb primary scaffold covering B73 chr4 positions 18--250 Mb, with the short arm tip (0.6--17 Mb) on a separate 17.2 Mb scaffold---a split confirmed by AnchorWave \citep{song2022} alignments to both B73 and PT.
Gene annotations were transferred from B73 via Liftoff \citep{shumate2021}.

To delimit the recombination breakpoints of \invfour, we performed CDS-anchored whole-genome alignments with AnchorWave \citep{song2022} on chromosome~4.
...
To characterize the sequence composition of these gene-poor breakpoint segments, we generated self-versus-self dotplots using LAST \citep{kielbasa2011}.
...
The repeat annotation available in MaizeGDB \citep{Woodhouse2025-za} for B73 identified the tandem arrays at the breakpoints as chromosomal knob-associated repeats of the \textit{knob180} and \textit{TR-1} classes.
...
We therefore retrieved the original \textit{knob180} \citep{dennis1984} and \textit{TR-1} \citep{hsu2003} reference sequences and performed BLAST+ \citep{camacho2009} megablast searches against each chromosome~4, both to capture these additional copies in B73 and to extend the analysis to PT, TIL18, and the Mi21 NIL, which lack public repeat annotations (see Methods).
...
\begin{figure*}[!ht]
\centering
\includegraphics[width=0.9\linewidth]{figs/WGCNA_module_perturbation.png}
